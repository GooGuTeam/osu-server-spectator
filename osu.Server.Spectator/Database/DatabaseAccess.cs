...
namespace osu.Server.Spectator.Database
{
  public class DatabaseAccess : IDatabaseAccess
  {
    ...
    public async Task<phpbb_zebra?> GetUserRelation(int userId, int zebraId)
    {
      ...
      return await connection.QuerySingleOrDefaultAsync<phpbb_zebra?>("SELECT * FROM `relationship` WHERE `user_id` = @UserId AND `zebra_id` = @ZebraId", new
      {
        UserId = userId,
        ZebraId = zebraId
      });
    ...
    public async Task<bool> GetUserAllowsPMs(int userId)
    {
      ...
      return await connection.QuerySingleOrDefaultAsync<bool>("SELECT `pm_friends_only` FROM `lazer_users` WHERE `id` = @UserId", new
      {
        UserId = userId
      });
    }
    ...
    public async Task<matchmaking_pool_beatmap?> GetMatchmakingPoolBeatmapAsync(uint poolId, int beatmapId, string mods)
    {
      ...
      return await connection.QuerySingleOrDefaultAsync<matchmaking_pool_beatmap>("SELECT p.*, b.playmode, b.checksum, b.difficulty_rating FROM `matchmaking_pool_beatmaps` p "
                                                                                  + "JOIN `beatmaps` b ON p.beatmap_id = b.id "
    ...
    public async Task UpdateMatchmakingPoolBeatmapRatingAsync(matchmaking_pool_beatmap beatmap)
    {
      ...
      await connection.ExecuteAsync("INSERT INTO `matchmaking_pool_beatmaps` (pool_id, beatmap_id, mods, rating, rating_sig) "
    ...
    public async Task<database_beatmap[]> GetMatchmakingGlobalPoolBeatmapsAsync(int rulesetId, int variant)
    {
      ...
      string modeString = rulesetId switch
      {
        0 => "osu",
        1 => "taiko",
        2 => "fruits",
