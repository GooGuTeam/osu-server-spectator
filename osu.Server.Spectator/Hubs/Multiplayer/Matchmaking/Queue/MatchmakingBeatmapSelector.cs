        ...
        public static async Task<MatchmakingBeatmapSelector> Initialise(matchmaking_pool pool, IDatabaseFactory dbFactory)
        {
            using (var db = dbFactory.GetInstance())
            {
                ...
                if (beatmaps.Length == 0)
                {
                    ...
                    beatmaps = globalBeatmaps.Select(b => new matchmaking_pool_beatmap { pool_id = pool.id, beatmap_id = b.beatmap_id, checksum = b.checksum, difficulty_rating = b.difficulty_rating })
                                             .ToArray();
                }
